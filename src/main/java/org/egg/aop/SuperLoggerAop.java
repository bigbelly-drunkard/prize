...
public class SuperLoggerAop {
    ...
    /**
     * 查找方法是否含有重载方法 必须为false(因为重载方法 ----请求中的参数a如果为空，就无法获取a的类名了，导致全不能用了)
     */
...
//            "!execution(* org.egg.service.impl.WechatServiceImpl.*(..)))")
...
//            不能影响流程
            LOGGER.error("doBeforeAdvice发生异常 e={}",e);
        }
    }
        ...
        if (joinPoint.getArgs() != null) {
            for (Object o : joinPoint.getArgs()) {
                if (o instanceof HttpServletRequest) {
                    ...
                    // 没辙: 2017/11/29  请求中的参数a如果为空，就无法获取a的类名了，导致全不能用了
